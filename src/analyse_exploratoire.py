...
Cette classe permet de réaliser une analyse exploratoire des données pour mieux comprendre la composition
du dataset ainsi que le type de variables et leurs distributions.
"""
class AnalyseEploratoireDonnees:
    ...
    # Cette méthode permet d'afficher le nombre de lignes et de colonnes du dataset
    ...
    # Cette méthode renvoie le pourcentage de valeurs manquantes au niveau de chaque variable(colonne)
    def pourcentage_val_manquantes(self):
        return ((self.data.isna().sum()/self.data.shape[0]).sort_values(ascending=False))*100
    
    #Cette méthode permet de renvoyer des statistiques des variables numériques (mean - median - min- max ...)
    def statistiques_descriptives(self):
        return self.data.describe()
    
    # Cette méthode permet de tracer l'histogramme et la ditribution des variables numériques
    ...
    #Cette méthode permet de tracer un heatmap pour visualiser les corrélations entre les variables    
    def matrice_correlation(self):
        plt.figure(figsize=(8, 6)) 
        sns.heatmap(self.data.corr(), annot=True, cmap='coolwarm', fmt=".2f")
        plt.show()

    # Cette méthode renvoie les modalités qui constituent les variables catégorielle
