...
class PretraitementsDonnees:
    ...
    # Cette méthode transforme chaque variable catégorielle en codes numériques, 
    # où chaque catégorie unique reçoit un entier unique
    def encodage_var_categorielles(self, categorical_columns):
        df = self.data.copy()
        for col in categorical_columns:
            df[col] = df[col].astype("category")
            # Transformer une colonne catégorielle dans un DataFrame pandas en une 
            # représentation numérique
            df[col] = df[col].cat.codes
        return df

    # Cette méthode permet de standardiser les données numériques pour les rendre 
    # utilisables lors de l'étape 3 du clustering
